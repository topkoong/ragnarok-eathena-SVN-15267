...
#include "pc.h"
#include "quest.h"
#include "fakeplayer.h" // fakeplayer_count()
...
#define PVP_CALCRANK_INTERVAL 1000	// PVPùùù?vùZù??u
...
/*==========================================
 * session idù?ù?ùù
 * charùIùùù?ùùù?ùùùùXùe?ù^ùXùù?ù
 *------------------------------------------*/
...
int pc_modifybuyvalue(struct map_session_data *sd,int orig_value)
{
	...
	if((skill=pc_checkskill(sd,MC_DISCOUNT))>0)	// ùfùBùXùJùEùùùg
		rate1 = 5+skill*2-((skill==10)? 1:0);
	if((skill=pc_checkskill(sd,RG_COMPULSION))>0)	// ùRùùùpùùùVùùùùùfùBùXùJùEùùùg
		rate2 = 5+skill*4;
...
int pc_modifysellvalue(struct map_session_data *sd,int orig_value)
{
	...
	if((skill=pc_checkskill(sd,MC_OVERCHARGE))>0)	// ùI?ùo?ù`ùù?ùW
...
/*==========================================
 * ùAùCùeùùù??ùùùùb?AùVùùùùùAùCùeùùùùùùùgùùùùùA
 * 3ùùù?ùùùù?ùùùù?ùmùF
 *------------------------------------------*/
...
/*==========================================
 * ùAùCùeùùùùTùùù?AùCùùùfùbùNùXùù?ù
 *------------------------------------------*/
...
/*==========================================
 * ùAùCùeùùù?ùùBùù?ù?ùitemù\ùù?ùù?ùùù??ù
 *------------------------------------------*/
...
/*==========================================
 * ùJ?ùgùAùCùeùùù?ùùBùù?ù?ùitemù\ùù?ùù?ùùù??ù
 *------------------------------------------*/
...
/*==========================================
 * ùJ?ùg?ù?AùCùeùù?ùmùF(ùù?ù?ùùùùù?ù)
 *------------------------------------------*/
...
int pc_randomwarp(struct map_session_data *sd, clr_type type)
{
	...
	if (map[sd->bl.m].flag.noteleport)	// ùeùùù|?ùgù?~
		return 0;
...
//
/*==========================================
 * ùXùLùùùù?ùù ùùùLùùù?ùùùù?Lvùùù?ù
 *------------------------------------------*/
...
/*==========================================
 * ùùùù?ùXù?ùùXùLùùùù??ù`ùFùbùN
 * ùù?ùF
 *   struct map_session_data *sd	ùZùbùVùùùùùf?ù^
 *   int nameid						?ùùùiID
 * ù?ùlùF
 *   0		?ùXù?ù
 *   -1		ùXùLùùùùùù
 ...
 * ùE?ùX
 * ùù?	job ùEùù 0ù`23
 *		upper ù?ù 0, ?ùù 1, ù{ùq 2, ùùù??ù -1
...
int pc_jobchange(struct map_session_data *sd,int job, int upper)
{
	...
	for(i=0;i<EQI_MAX;i++) {
		if(sd->equip_index[i] >= 0)
			if(!pc_isequip(sd,sd->equip_index[i]))
				pc_unequipitem(sd,sd->equip_index[i],2);	// ?ùùùOùù
	}
...
{
	if( flag ){
		if( pc_checkskill(sd,HT_FALCON)>0 )	// ùtù@ùùùRùùù}ùXù^ùù?ùXùLùùùùùù
...
{
	if( flag ){
		if( pc_checkskill(sd,KN_RIDING) > 0 ) // ùùùCùfùBùùùOùXùLùùùùùù
...
/*==========================================
 * ùAùCùeùùùùindexùù?ùùùlù?ùùù
 * ? ùùùiùù?ùùù?\ù`ùFùbùNùùsù?ù
 *------------------------------------------*/
...
/*==========================================
 * sdù?ùùùùùù?ùù?(?ùùù??ù?ùùùùùchar_idùù?ù)
 *------------------------------------------*/
...
/*==========================================
 * sdùùdstsdù?ùùù(dstsdùùsdù?ùùù?ùùùùùùùb?sùù)
 *------------------------------------------*/
...
int pc_autosave(int tid, unsigned int tick, int id, intptr_t data)
{
	...
	{
		if(sd->state.fakeplayer) //Fake players are never persisted.
			continue;
	...
	//Base the cycle on the count of REAL players only; fake players would
	//otherwise collapse the interval and burn CPU iterating thousands of them.
	interval = autosave_interval/(map_usercount()-fakeplayer_count()+1);
