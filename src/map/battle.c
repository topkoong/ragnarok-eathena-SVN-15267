...
int battle_calc_damage(struct block_list *src, struct block_list *bl, struct Damage *d, int damage, int skill_num, int skill_lv)
{
	...
	// new code snippets are shown below
	if (sc && sc->data[SC_RAID] && damage > 0)
	{
		/*
		 * Renewal / iRO Sightless Mind vulnerability effect:
		 *
		 * While SC_RAID is active on the victim:
		 * - normal targets take 30% more incoming damage
		 * - boss targets   take 15% more incoming damage
		 *
		 * This should apply to positive damage only.
		 */
		if (status_get_mode(bl) & MD_BOSS)
			damage = damage * 115 / 100; // boss: +15%
		else
			damage = damage * 130 / 100; // normal target: +30%
	}
	...
	if (sc && sc->count)
	{
		...
		if (sc->data[SC_DODGE] && !sc->opt1 &&
			(flag & BF_LONG || sc->data[SC_SPURT]) && rand() % 100 < 20)
		{
			if (sd && pc_issit(sd))
		...
		if ((sce = sc->data[SC_REJECTSWORD]) && flag & BF_WEAPON &&
			...
			(src->type != BL_PC || (((TBL_PC *)src)->status.weapon == W_DAGGER ||
...
static struct Damage battle_calc_weapon_attack(struct block_list *src, struct block_list *target, int skill_num, int skill_lv, int wflag)
{
	...
	case LK_SPIRALPIERCE:
		if (!sd)
			n_ele = false; // forced neutral for monsters
		break;
	}
	...
	{																																			  // Check for double attack.
		if (((skill_lv = pc_checkskill(sd, TF_DOUBLE)) > 0 && sd->weapontype1 == W_DAGGER) || (sd->double_rate > 0 && sd->weapontype1 != W_FIST)) // Will fail bare-handed
	...
	if (skill_num == RG_BACKSTAP)
	{
		// New Back Stab hit-count behavior:
		// dagger = 2 hits, everything else = 1 hit
...
#define ATK_RATE(a)                              \
	{                                            \
		wd.damage = wd.damage * (a) / 100;       \
		if (flag.lh)                             \
			wd.damage2 = wd.damage2 * (a) / 100; \
	}
#define ATK_RATE2(a, b)                          \
	{                                            \
		wd.damage = wd.damage * (a) / 100;       \
		if (flag.lh)                             \
			wd.damage2 = wd.damage2 * (b) / 100; \
	}
// Adds dmg%. 100 = +100% (double) damage. 10 = +10% damage
#define ATK_ADDRATE(a)                            \
	{                                             \
		wd.damage += wd.damage * (a) / 100;       \
		if (flag.lh)                              \
			wd.damage2 += wd.damage2 * (a) / 100; \
	}
#define ATK_ADDRATE2(a, b)                        \
	{                                             \
		wd.damage += wd.damage * (a) / 100;       \
		if (flag.lh)                              \
			wd.damage2 += wd.damage2 * (b) / 100; \
...
#define ATK_ADD2(a, b)       \
	{                        \
		...
		else
		{
			...
			case AS_SONICBLOW:
				/*
				 * May 1, 2020 iRO rebalance:
				 * New formula = (200 + 100 * skill_lv)% ATK
				 *
				 * In this old eAthena battle.c style, final damage is:
				 *   100% base + skillratio
				 *
				 * So to get:
				 *   200 + 100 * skill_lv
			...
			// 	break;
			// The new code snippets are provided below
			case RG_BACKSTAP:
				// Handle Back Stab damage ratio.
				//
				// In this old battle.c style, "skillratio" is an ADDITION to the base 100%.
				// So:
				//   skillratio += 200 + 40 * skill_lv
				// means the real final ratio becomes:
				//   100% base + (200 + 40*lv)
				//
				// Since you have not asked to change Back Stab's damage formula here,
				// we keep the same classic formula logic for now.
				//
				// The only special case is when the attacker uses a bow and
				// backstab_bow_penalty is enabled in battle config.

				if (sd && sd->status.weapon == W_BOW && battle_config.backstab_bow_penalty)
				{
					// If:
					// - sd exists => attacker is a player
					// - attacker's weapon is a bow
					// - server config says bow penalty should apply
					//
					// Then use half of the usual Back Stab ratio.
					skillratio += (200 + 40 * skill_lv) / 2;
				}
				else
				{
					// Otherwise use the normal Back Stab ratio.
					skillratio += 200 + 40 * skill_lv;
				}
			...
			// 	skillratio += 40 * skill_lv;
			// 	break;
			// new code snippets are here
			// base damage already starts from 100%
			// old + 40*lv means old final formula = 100 + 40*lv
			// new target formula is 50 + 150*lv
			// so the correct old-style translation is -50 + 150*lv
			case RG_RAID:
			...
			case CR_SHIELDBOOMERANG:
				 ...
				 *   (80 * skill_lv)% ATK
				 *
				 * Old eAthena battle.c style:
				 * final damage = 100% base + skillratio
				 *
				 * Therefore:
			...
			case MO_FINGEROFFENSIVE:
				/*
				 * Rebalance:
				 *   (600 + 200 * skill_lv)% ATK
				 *
				 * Old style translation:
				 *   skillratio += (600 + 200 * skill_lv) - 100
				 *              += 500 + 200 * skill_lv
				 */
		...
		case AS_SONICBLOW:
			/*
			 * Existing Soul Linker bonus for Assassin Spirit.
			 * Keep current behavior in this tree unless you want to
			 * rebalance that separately.
			 */
			if (sc && sc->data[SC_SPIRIT] &&
				sc->data[SC_SPIRIT]->val2 == SL_ASSASIN)
				ATK_ADDRATE(map_flag_gvg(src->m) ? 25 : 100);

			/*
			 * Existing Sonic Acceleration bonus.
			 * Keep as-is unless you want to rebalance that skill too.
			 */
			if (sd && pc_checkskill(sd, AS_SONICACCEL) > 0)
				ATK_ADDRATE(10);
			...
			/*
			 * May 1, 2020 iRO rebalance:
			 * Sonic Blow deals 50% more damage if the target has
			 * less than 50% HP remaining.
			 */
			if (status_get_hp(target) * 2 < status_get_max_hp(target))
				ATK_ADDRATE(50);
		...
		case MO_FINGEROFFENSIVE:
			/*
			 * Rebalance:
			 * Throw Spirit Sphere deals 50% more damage to rooted targets.
			 *
			 * In this old tree, "Root" maps most closely to SC_BLADESTOP.
			 */
			if (status_get_sc(target) && status_get_sc(target)->data[SC_BLADESTOP])
				ATK_ADDRATE(50);
			break;
		...
		case MO_INVESTIGATE:
			/*
			 * Rebalance:
			 * Occult Impaction deals 50% more damage to rooted targets.
			 *
			 * In this old tree, "Root" maps most closely to SC_BLADESTOP.
			 */
			if (status_get_sc(target) && status_get_sc(target)->data[SC_BLADESTOP])
		...
		if (sd)
		{
			...
			{ // Ignore Defense?
				if (!flag.idef && (sd->right_weapon.ignore_def_ele & (1 << tstatus->def_ele) ||
								   sd->right_weapon.ignore_def_race & (1 << tstatus->race) ||
								   sd->right_weapon.ignore_def_race & (is_boss(target) ? 1 << RC_BOSS : 1 << RC_NONBOSS)))
					flag.idef = 1;

				if (!flag.idef2 && (sd->left_weapon.ignore_def_ele & (1 << tstatus->def_ele) ||
		...
		{ // SC skill damages
			if (sc->data[SC_AURABLADE])
			{
				/*
				 * Rebalance:
				 * Aura Blade flat bonus = Base Level * (3 + skill level)
				 */
				ATK_ADD(status_get_lv(src) * (3 + sc->data[SC_AURABLADE]->val1));
			}
		}
	...
	{                   \
		...
		default:
		{
			...
			case WZ_VERMILION:
				/*
				 * Rebalance:
				 *   (400 + 100 * skill_lv)% MATK
				 *
				 * Old style translation:
				 *   skillratio += (400 + 100 * skill_lv) - 100
				 *              += 300 + 100 * skill_lv
				 */
				skillratio += 300 + 100 * skill_lv;
				break;
		...
		if (sd)
		{
			...
			// Ignore Defense?
			if (!flag.imdef && (sd->ignore_mdef_ele & (1 << tstatus->def_ele) ||
		...
		{ // Apply the physical part of the skill's damage. [Skotlex]
			...
			/*
			 * Rebalance:
			 * Remove Grand Cross self damage.
			 * HP cost on casting stays in skill requirements.
			 */
			if (src == target)
				ad.damage = 0;
		}
...
enum damage_lv battle_weapon_attack(struct block_list *src, struct block_list *target, unsigned int tick, int flag)
{
	...
	if (tsc)
	{
		if (tsc->data[SC_POISONREACT] &&
			(rand() % 100 < tsc->data[SC_POISONREACT]->val3 || sstatus->def_ele == ELE_POISON) &&
...
static const struct _battle_data
{
	...
	{
		"gm_can_party",
		&battle_config.gm_can_party,
		0,
		0,
		1,
	},
	{
		"gm_cant_party_min_lv",
		&battle_config.gm_cant_party_min_lv,
		20,
		0,
		100,
	},
	...
	},
	{
		...
		&battle_config.pet_equip_min_friendly,
		900,
		0,
		950,
	},
	...
	{
		"agi_penalty_count_lv",
		&battle_config.agi_penalty_count_lv,
		ATK_FLEE,
		0,
		INT_MAX,
	},
	...
	{
		"gvg_eliminate_time",
		&battle_config.gvg_eliminate_time,
		7000,
		0,
		INT_MAX,
	},
	...
	{
		"castle_defense_rate",
		&battle_config.castle_defense_rate,
		100,
		0,
		100,
	},
	{
		"gm_cant_drop_min_lv",
		&battle_config.gm_cant_drop_min_lv,
		1,
		0,
		100,
	},
	{
		"gm_cant_drop_max_lv",
		&battle_config.gm_cant_drop_max_lv,
		0,
		0,
		100,
	},
	...
	{
		"cell_stack_limit",
		&battle_config.cell_stack_limit,
		1,
		1,
		255,
	},
	...
	},
	{
		...
		&battle_config.any_warp_GM_min_level,
		20,
		0,
		100,
	},
	...
	{
		"copyskill_restrict",
		&battle_config.copyskill_restrict,
		2,
		0,
		2,
	},
	...
	{
		"gm_viewequip_min_lv",
		&battle_config.gm_viewequip_min_lv,
		0,
		0,
		99,
	},
	...
	{
		"gm_check_minlevel",
		&battle_config.gm_check_minlevel,
		60,
		0,
		100,
	},
