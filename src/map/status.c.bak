...
void initChangeTables(void)
{
	...
	set_sc(AL_INCAGI, SC_INCREASEAGI, SI_INCREASEAGI, SCB_AGI | SCB_SPEED | SCB_ASPD);
	set_sc(AL_DECAGI, SC_DECREASEAGI, SI_DECREASEAGI, SCB_AGI | SCB_SPEED);
	set_sc(AL_CRUCIS, SC_SIGNUMCRUCIS, SI_SIGNUMCRUCIS, SCB_DEF);
	set_sc(AL_ANGELUS, SC_ANGELUS, SI_ANGELUS, SCB_DEF2 | SCB_MAXHP);
	set_sc(AL_BLESSING, SC_BLESSING, SI_BLESSING, SCB_STR | SCB_INT | SCB_DEX | SCB_HIT);
	set_sc(AC_CONCENTRATION, SC_CONCENTRATE, SI_CONCENTRATE, SCB_AGI | SCB_DEX);
	set_sc(TF_HIDING, SC_HIDING, SI_HIDING, SCB_SPEED);
	add_sc(TF_POISON, SC_POISON);
	set_sc(KN_TWOHANDQUICKEN, SC_TWOHANDQUICKEN, SI_TWOHANDQUICKEN, SCB_ASPD | SCB_HIT | SCB_CRI);
	...
	set_sc(BS_ADRENALINE, SC_ADRENALINE, SI_ADRENALINE, SCB_ASPD | SCB_HIT);
	...
	set_sc(MC_LOUD, SC_LOUD, SI_LOUD, SCB_STR | SCB_BATK);
	...
	add_sc(RG_RAID, SC_STUN); // This keeps the old Raid-to-Stun association.
	/*
	New code snippets below
	This registers your new SC_RAID status:
		no icon
		no stat recalculation flag
		but valid for the status system and for battle.c to check
	*/
	set_sc(RG_RAID, SC_RAID, SI_BLANK, SCB_NONE);
...
int status_damage(struct block_list *src, struct block_list *target, int hp, int sp, int walkdelay, int flag)
{
	...
	case BL_HOM:
		...
		break;
	case BL_MER:
		mercenary_damage((TBL_MER *)target, src, hp, sp);
		break;
	}
	...
	case BL_HOM:
		...
		break;
	case BL_MER:
		flag = mercenary_dead((TBL_MER *)target, src);
		break;
...
int status_percent_change(struct block_list *src, struct block_list *target, signed char hp_rate, signed char sp_rate, int flag)
{
	...
	if (hp_rate > 99)
		...
		hp = status->hp > 10000 ? hp_rate * (status->hp / 100) : (hp_rate * status->hp) / 100;
	else if (hp_rate < -99)
		hp = status->max_hp;
	else if (hp_rate < 0)
		hp = status->max_hp > 10000 ? (-hp_rate) * (status->max_hp / 100) : (-hp_rate * status->max_hp) / 100;
	if (hp_rate && !hp)
		hp = 1;
...
int status_check_skilluse(struct block_list *src, struct block_list *target, int skill_num, int flag)
{
	...
	if (src && skill_num == AL_TELEPORT && map_getcell(src->m, src->x, src->y, CELL_CHKLANDPROTECTOR) && !(status->mode & MD_BOSS) && (src->type != BL_PC || ((TBL_PC *)src)->skillitem != skill_num))
		return 0;
	...
	if (sc && sc->count)
	{
		...
		if (
			(sc->data[SC_TRICKDEAD] && skill_num != NV_TRICKDEAD) || (sc->data[SC_AUTOCOUNTER] && !flag) || (sc->data[SC_GOSPEL] && sc->data[SC_GOSPEL]->val4 == BCT_SELF && skill_num != PA_GOSPEL) || (sc->data[SC_GRAVITATION] && sc->data[SC_GRAVITATION]->val3 == BCT_SELF && flag != 2))
			return 0;
	...
	if (sc && sc->option)
	{
		...
		// 			return 0;
		// 	}
		// new code snippets are listed here
		if (sc->option & OPTION_HIDE)
			switch (skill_num)
			{ // Skills that are still allowed while the user is hidden.
			case TF_HIDING:
				// Re-casting Hiding itself is allowed.
				break;

			case AS_GRIMTOOTH:
				// Grimtooth is designed to work from hiding.
				break;

			case RG_RAID:
				// Raid should still be usable while hiding.
				// This keeps Rogue Raid working as a hidden-area attack.
				break;

			case NJ_SHADOWJUMP:
				// Ninja shadow movement skill.
				break;

			case NJ_KIRIKAGE:
				// Ninja hidden-movement/attack skill.
				break;

			default:
				// Everything else is blocked while hidden.
				//
				// This means RG_BACKSTAP stays disallowed from Hiding,
				// because we intentionally do NOT list it here.
				//
				// Non-player units may still bypass this depending on server logic.
				if (!skill_num || src->type == BL_PC)
					return 0;
			}
		// old code
...
void status_calc_regen_rate(struct block_list *bl, struct regen_data *regen, struct status_change *sc)
{
	...
	if (
		(sc->data[SC_POISON] && !sc->data[SC_SLOWPOISON]) || (sc->data[SC_DPOISON] && !sc->data[SC_SLOWPOISON]) || sc->data[SC_BERSERK] || sc->data[SC_TRICKDEAD] || sc->data[SC_BLEEDING]) // No regen
		regen->flag = 0;

	if (
		sc->data[SC_DANCING] || (bl->type == BL_PC && (((TBL_PC *)bl)->class_ & MAPID_UPPERMASK) == MAPID_MONK && (sc->data[SC_EXTREMITYFIST] || (sc->data[SC_EXPLOSIONSPIRITS] && (!sc->data[SC_SPIRIT] || sc->data[SC_SPIRIT]->val2 != SL_MONK)))) || sc->data[SC_MAXIMIZEPOWER]) // No natural SP regen
		regen->flag &= ~RGN_SP;
...
void status_calc_bl_main(struct block_list *bl, /*enum scb_flag*/ int flag)
{
	...
	if (flag & SCB_MAXHP)
	{
		...
		/*
		 * Rebalance:
		 * Angelus also increases Max HP by 50 per skill level.
		 *
		 * Apply this after the normal max-HP recalculation so it behaves
		 * as a flat bonus on top of the recalculated value.
		 */
		if (sc && sc->data[SC_ANGELUS])
			status->max_hp += 50 * sc->data[SC_ANGELUS]->val1;

		/*
		 * Keep the server-side max HP cap consistent for both PCs and non-PCs.
		 */
		if (status->max_hp > (unsigned int)battle_config.max_hp)
...
static signed short status_calc_critical(struct block_list *bl, struct status_change *sc, int critical)
{
	...
	if (sc->data[SC_TWOHANDQUICKEN])
		critical += 10 * (2 + sc->data[SC_TWOHANDQUICKEN]->val1);
...
static signed short status_calc_hit(struct block_list *bl, struct status_change *sc, int hit)
{
	...
	if (sc->data[SC_BLESSING])
		hit += 2 * sc->data[SC_BLESSING]->val1;
	if (sc->data[SC_ADRENALINE])
		hit += 5 + 3 * sc->data[SC_ADRENALINE]->val1;
	if (sc->data[SC_TWOHANDQUICKEN])
...
static signed short status_calc_def2(struct block_list *bl, struct status_change *sc, int def2)
{
	...
	if (sc->data[SC_JOINTBEAT])
		def2 -= def2 * (sc->data[SC_JOINTBEAT]->val2 & BREAK_SHOULDER ? 50 : 0) / 100 + def2 * (sc->data[SC_JOINTBEAT]->val2 & BREAK_WAIST ? 25 : 0) / 100;
	if (sc->data[SC_FLING])
...
static short status_calc_aspd_rate(struct block_list *bl, struct status_change *sc, int aspd_rate)
{
	...
	if (!sc->data[SC_QUAGMIRE])
	{
		...
		if (sc->data[SC_INCREASEAGI] && max < 10 * sc->data[SC_INCREASEAGI]->val1)
			max = 10 * sc->data[SC_INCREASEAGI]->val1;

		if (sc->data[SC_TWOHANDQUICKEN] && max < 100)
			max = 100;
		...
		if (sc->data[SC_ADRENALINE2] && max < 100)
			max = 100;

		if (sc->data[SC_ADRENALINE] && max < 100)
			max = 100;

		if (sc->data[SC_SPEARQUICKEN] && max < 100)
			max = 100;
...
void status_set_viewdata(struct block_list *bl, int class_)
{
	...
	if (vd && vd->cloth_color && ((vd->class_ == JOB_WEDDING && battle_config.wedding_ignorepalette) || (vd->class_ == JOB_XMAS && battle_config.xmas_ignorepalette) || (vd->class_ == JOB_SUMMER && battle_config.summer_ignorepalette)))
		vd->cloth_color = 0;
}
...
int status_change_start(struct block_list *bl, enum sc_type type, int rate, int val1, int val2, int val3, int val4, int tick, int flag)
{
	...
	if ((sce = sc->data[type]))
	{
		...
		case SC_ADRENALINE:
			 ...
			 *
			 * In this old tree, aspd-rate helpers use these stored values.
			 * We standardize both to a 10% reduction target here.
			 */
			val3 = 100;
	...
	if (!(flag & 4)) //&4 - Do not parse val settings when loading SCs
		...
		case SC_SPEARQUICKEN:
			 ...
			 * Delay after attack reduced by 10%.
			 *
			 * In the latest file, status_calc_aspd_rate() already treats
			 * SC_SPEARQUICKEN as a flat 100-value quicken effect.
			 *
			 * Set val2 to 100 as well so the stored status data matches the
			 * rebalance target and stays internally consistent.
			 */
			val2 = 100;
			break;
		case SC_DANCING:
		...
		case SC_OVERTHRUST:
			 ...
			 * Lv 5   = +15%
			 *
			 * val2 still keeps the old "self cast / received from others" marker.
			 */
			if (val1 <= 2)
				val3 = 5;
			else if (val1 <= 4)
...
int status_change_end_(struct block_list *bl, enum sc_type type, int tid, const char *file, int line)
{
	...
	case SC_SUMMER:
		...
		{ // Load data from sd->status.* as the stored values could have changed.
			...
			sc->option &= type == SC_WEDDING ? ~OPTION_WEDDING : type == SC_XMAS ? ~OPTION_XMAS
																				 : ~OPTION_SUMMER;
	...
	case SC_CLOSECONFINE:
		if (sce->val2 > 0)
		{
			...
			int range = 1 + skill_get_range2(bl, status_sc2skill(type), sce->val1) + skill_get_range2(bl, TF_BACKSLIDING, 1); // Since most people use this to escape the hold....
	...
	case SC_DPOISON:
		if (--(sce->val3) > 0)
		{
			if (!sc->data[SC_SLOWPOISON])
			{
				...
				if (flag)
					return 0; // target died, SC cancelled already.
			}
			sc_timer_next(1000 + tick, status_change_timer, bl->id, data);
			return 0;
		}
